...
export const IndependentCustomCard: React.FC<IndependentCustomCardProps> = ({
  ...
  const openCard = (e: React.MouseEvent) => {
    ...
    // Dimensioni aperte (responsive ma con max come da tua richiesta originale)
    const openW = Math.min(1300, window.innerWidth * 1.1);
    const openH = Math.min(820, window.innerHeight * 0.95);

    // Passa a fixed e blocca dimensioni correnti
    gsap.set(card, {
      position: "fixed",
      left: `${rect.left}px`,
      top: `${rect.top}px`,
    ...
    // Calcola centro con dimensioni finali
    ...
    // Torna a dimensione iniziale per animare la transizione
  ...
  return (
    <>
      ...
      <div
        ...
        className="cloud-glass-card position-absolute"
        style={{
          zIndex: isOpen ? 10001 : 30,
          transformStyle: "preserve-3d",
          willChange: "transform, width, left, top",
          cursor: isOpen ? "default" : "pointer",
          transition: "border-color 0.3s ease",
          width: isOpen ? undefined : style.width ?? "280px",
          height: isOpen ? undefined : style.height ?? "140px",
          ...(isOpen
            ? {
                alignItems: "stretch",
        ...
        {!isOpen && (
          <div className="w-100 h-100 p-3 d-flex flex-column justify-content-between">
            <div className="d-flex align-items-center justify-content-between">
              <span className="font-monospace fw-bold text-uppercase fs-7 text-muted">
                {config.customCardTitle || "Custom Info"}
              </span>
              <Maximize2 size={14} className="text-muted" />
            </div>
            <div className="text-truncate text-secondary small">
              {config.customCardText ||
                (hasImg ? "Immagine allegata" : "Clicca per aprire...")}
            </div>
          </div>
        )}
          ...
          <div className="d-flex flex-column w-100 h-100 position-relative overflow-hidden">
            ...
            <div className="d-flex align-items-center justify-content-between px-3 border-3">
              <div className="d-flex align-items-center w-100 mt-3 gap-2 mb-3 text-muted font-monospace border-bottom pb-2">
                ...
                <span className="font-monospace fw-bold text-uppercase fs-5">
                  {config.customCardTitle || "Custom Info"}
                </span>
              </div>
            </div>
            ...
            <div className="d-flex flex-column flex-grow-1 overflow-hidden">
              ...
              {!hasImg && hasText && (
                <div className="w-100 h-100 d-flex align-items-center justify-content-center p-5 text-center">
                  <p className="fs-5 fw-medium text-dark lh-base m-0">
                    {renderText(config.customCardText)}
                  </p>
                </div>
              )}
              ...
              {hasImg && hasText && (
                <>
                  ...
                  <div
                    className="w-100 d-flex align-items-center justify-content-center px-5 text-start overflow-auto"
                    style={{ height: "35%" }}
                  >
                    <p className="fs-5 fw-medium text-dark lh-base m-0">
                      {renderText(config.customCardText)}
